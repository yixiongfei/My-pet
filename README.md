...
**当前阶段：Body / Core 已可日常使用，Agent 与知识库能力继续按路线图补齐。** 完整实现状态见 [docs/](docs/README.md) 与 [路线图](docs/06-roadmap.md)。
...
pnpm clean                              # 本机只留正式版：清掉 debug/release 编译缓存、dist、旧安装包、日志与残留副本
```
...
- **放歌她就跳**：Spotify 在播的时候（她只看 Spotify 窗口标题，不碰音频）她放下手头的活跟着跳，歌停了回去干活；到点吃饭睡觉照旧。对她说「放首歌」会打开 Spotify 开始播放。状态全满时偶尔飞吻，闲着会伸懒腰、喝茶；按住脸拖是捏脸；吃饭偶尔是麦当劳。
- 她说的话浮在**头顶的气泡**里；「帮我设个番茄钟，学习一个小时」「十分钟后叫我」会在头顶挂一个**倒计时环**，到点提醒。
- 设置里可以调**显示大小**（200–800 px）、是否**始终置顶**，写她的**名字 / 背景 / 形象 / 性格 / 说话方式**。托盘点「送她礼物」会打开带图片的礼品页，由你挑一件再送；折叠的「状态与调试」可查看数值、记忆和计时器，不再混入开发说明。她自己吃饭或喝水时，会从买得起、能满足当前需求的同类里随机挑选。
...
- **她会主动开口，但有分寸**：装了本机「知识库」App 的话，她会读它的日程和到期复习（只读，不改），早上第一次见到你说一句今天的安排，每件事开始前 15 分钟提一句「该开始了」；你离开半小时以上再回来，她会接一句待办或说说你不在时她干了什么；连着坐两个小时她会先小声嘀咕一句，再半小时才劝你起来走走；平时隔半小时左右自己嘀咕一句（小气泡、轻声、不占「搭话」次数）。**搭话一天最多 6 次**，你回了话明天多一次、连着不理明天少两次；说「别烦我」「安静一小时」立刻静音（这是规则，不经模型）。夜里、她睡着、你不在电脑前、刚从待机醒来都不说；同一件事不说第二遍；你没回的气泡三十秒自己收。托盘「今天有什么安排」随时能问，聊天里问「今晚学什么」她也答得上来。
