...
## 7. 窗口布局与气泡（2026-09-18 现状）

- pet 窗口高 = 宽 × 1.6：下面的正方形是立绘（500×500 参考系，命中掩码、触摸区域、拖拽锚点都只按它算），
  上面 0.6 倍边长的「头顶区」放**气泡**（尾巴指向头）和**倒计时环**（番茄钟 / 「学习一个小时」）。
  头顶区永远穿透，所以气泡上没有按钮：按头顶区高度决定最多几行（≤ 7），超出打省略号；流式输出时只显示尾巴。比例常量 `HEAD_ROOM`
  在 `lib.rs` 和 `PetCanvas.tsx` 各一份，改要一起改。
...
## 8. 行为树与动画池（2026-09-18）

Body 按这棵树挑画面。**Core 只决定「在做什么」和数值**（`state_machine.rs`），Body 决定「看起来在干什么」：

```
萝莉斯
├─ 紧急状态（Core：生理急需压过一切）
│   ├─ 体力耗尽 → Sleep     ├─ 严重饥饿 → Eat     └─ 严重口渴 → Drink
├─ 用户交互（Body：高优先级、临时，播完回到当前计划）
│   ├─ Gift → 拆礼物一次     ├─ Touch → 摸头 / 摸身     └─ Drag → 提起 / 挣扎 / 落地 / 侧挂
├─ 当前计划（Core 定活动，Body 在池里轮换，`animationPool.ts`）
│   ├─ Work  → 写文案 8–15 min · 清屏 5–10 · 直播 10–20 · 烧烤 6–12 · 修屏幕 6–12
│   ├─ Study → 看书 · 写字 · 研究 · 画画
│   ├─ Play  → 打游戏 · 删错误 · 跳绳 · 玩水
│   └─ Sleep → 入睡（start）· 熟睡（loop 随机变体）· 醒来（end）
└─ 无任务（Idle）→ 发呆 · 四处看 / 小动作（idel）· 走路 / 爬墙（pet_motion）
```

- 每段池动画有自己的驻留时长；**驻留没到不因轮换而换**，到了在同池里随机换一个不同的。刚进活动优先播 Core 指名的（`action.graph`）。
- 允许打断的只有三种：用户交互、生理急需（Core 换了活动）、活动自然结束（Core 换了活动）。被交互打断后回来接着播原来那段。
- 不在池里的活动（睡觉、发呆、吃喝、收礼）走 `CLIP_FOR` 的直接映射。

## 9. 原版移动资源接入状态（2026-09-18）
