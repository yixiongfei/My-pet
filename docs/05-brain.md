...
## 6. 主动提醒（`kb.rs` · `nudge.rs`）

第一个「主动行为」，全是确定性规则，模型不参与「什么时候开口」。
...
**什么时候闭嘴**（`lib.rs::check_nudges`，每分钟看一次）：提醒或台词总开关关了；23–8 点；她在睡觉；你 5 分钟内没动过键鼠（`GetLastInputInfo`，锁屏也算不在）；刚从待机醒来不到 3 分钟（心跳的时钟停摆检测）。
