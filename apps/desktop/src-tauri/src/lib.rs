...
fn apply(app: &AppHandle, event: &Event) {
    apply_with(app, event, false);
}

/// `quiet` = 判定结果不出气泡。对话里的使唤走这条：模型的回复就是她的回答，
/// 不该先蹦一句「好呀这就去」再来一段正文
fn apply_with(app: &AppHandle, event: &Event, quiet: bool) {
...
/// 此刻的状态快照。对话要拿它写「此刻」，台词要拿它看心情
pub(crate) fn pet_snapshot(app: &AppHandle) -> PetState {
    app.try_state::<Mutex<Pet>>()
        .and_then(|p| p.lock().ok().map(|p| p.state.clone()))
        .unwrap_or_default()
}
...
#[tauri::command]
fn request_action(app: AppHandle, target: String, minutes: Option<f32>) -> Option<Verdict> {
    apply(&app, &Event::Request { target, roll: roll(), minutes });
    let pet = app.state::<Mutex<Pet>>();
    pet.lock().ok().and_then(|p| p.last_verdict.clone())
}

/// 对话里的使唤：同一套判定，但不出气泡（chat.rs 会把结果写进模型的「此刻」）
pub(crate) fn request_action_quiet(app: &AppHandle, target: String, minutes: Option<f32>) -> Option<Verdict> {
    apply_with(app, &Event::Request { target, roll: roll(), minutes }, true);
    let pet = app.state::<Mutex<Pet>>();
    pet.lock().ok().and_then(|p| p.last_verdict.clone())
}
...
/// 面板看的：你让她做的事还剩多久
#[tauri::command]
fn get_directive(app: AppHandle) -> Option<Directive> {
    let pet = app.state::<Mutex<Pet>>();
    pet.lock().ok().and_then(|p| p.directive.clone())
}

/// 0–1 的随机数。不拉 rand 依赖——一个 splitmix64 够了，
/// 而且这里要的只是「不可预测」，不是密码学强度。
///
/// 进程全局一份状态，不用 thread_local：对话命令跑在 tokio 的任意工作线程上，
/// 每个线程各自用毫秒时间戳做种子的话，xorshift 的头几个输出全是 0.97 上下——
...
fn execute(app: &AppHandle, call: &ToolCall) -> ToolResult {
    ...
    match call.name.as_str() {
        ...
        "request_action" => {
            ...
            let minutes = call.input.get("minutes").and_then(|v| v.as_f64()).map(|v| v as f32);
            match request_action(app.clone(), target, minutes) {
...
fn give_gift(app: AppHandle, id: Option<String>) -> Result<String, String> {
    ...
    // 先发 gift 事件让 Body 播一遍拆礼物的动画，再说话：收礼必须当场有反应，不受台词间隔限制
    let _ = app.emit("pet:gift", GiftReceived { id, name: name.clone() });
    if let Some(a) = pet_snapshot(&app).action.as_ref().filter(|a| a.id == "gift") {
...
#[tauri::command]
async fn tts_status(app: AppHandle) -> Result<tts::TtsStatus, String> {
    let settings = chat::current_settings(&app)?;
    Ok(tts::status(&settings.voice).await)
}

/// 面板配台词要看的动作列表
#[tauri::command]
fn list_actions(cat: State<'_, Catalog>) -> Vec<lines::ActionInfo> {
    lines::list_actions(&cat)
}

/// 让模型按人设给某个动作写几句台词。用户挑着留，不自动保存
#[tauri::command]
async fn draft_action_lines(app: AppHandle, id: String, count: Option<usize>) -> Result<Vec<String>, String> {
    let settings = chat::current_settings(&app)?;
    let cat = app.state::<Catalog>();
    lines::draft_lines(&settings, &cat, &id, count.unwrap_or(5)).await
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // 语音服务比桌宠晚几秒起来；后台等它就绪后预热一次，第一句台词就不用等着色器编译
            if let Ok(s) = chat::current_settings(app.handle()) {
                tts::warmup(app.handle().clone(), s.voice);
            }
...
mod desktop_tests {
    ...
    #[test]
    fn roll_is_uniform_from_the_first_call() {
        // 老实现用时间戳直接当 xorshift 种子，头一个值几乎必然 > 0.95
        let first = roll();
