...
pub(crate) fn local_endpoint(value: &str) -> Result<String, String> {
...
fn validate_settings(mut settings: ChatSettings) -> Result<ChatSettings, String> {
    ...
    settings.voice = settings.voice.validated()?;
    settings.lines = settings.lines.validated()?;
    Ok(settings)
}

/// 现在生效的设置（校验过的）。台词、语音这些不在对话流程里的调用方也要它
pub fn current_settings(app: &AppHandle) -> Result<ChatSettings, String> {
    with_store(app, |store| validate_settings(store.settings()?))
}
...
/// 她此刻的处境：在做什么、身体怎么样、刚才有没有答应 / 拒绝用户什么。
/// 这是对话和身体之间唯一的桥：模型不读数值，只读这段话
#[derive(Clone, Debug, Default, PartialEq)]
pub struct Situation {
    /// 「你正在：学书法（因为该学习了）。心情不错，有点饿。」
    pub state: String,
    /// 刚处理过的使唤：「用户让你去玩，你答应了，会玩 30 分钟」
    pub note: Option<String>,
    /// 模型没回上话时兜底的那句（判定自带的台词）
    pub fallback: Option<String>,
}

fn level_word(v: f32, high: &'static str, mid: &'static str, low: &'static str, worst: &'static str) -> &'static str {
    if v >= 70.0 { high } else if v >= 40.0 { mid } else if v >= 20.0 { low } else { worst }
}

/// 把数值翻译成她自己会说的话。粗粒度就够了——模型不需要知道饱腹是 63 还是 58
pub fn describe_state(s: &PetState) -> String {
    let doing = match s.action.as_ref() {
        Some(a) => {
            let what = match a.food.as_ref() { Some(f) => format!("{}（{}）", a.name, f.name), None => a.name.clone() };
            format!("你正在：{what}，因为{}。", a.reason)
        }
        None => "你正闲着。".into(),
    };
    let mood = match s.mood {
        Mood::Happy => "心情很好",
        Mood::Nomal => "心情一般",
        Mood::PoorCondition => "状态不太好",
        Mood::Ill => "生病了",
    };
    let body = [
        level_word(s.strength, "精力充沛", "体力还行", "有点累", "累坏了"),
...
fn looks_like_aside(line: &str) -> bool {
    const OPENERS: [&str; 19] = [
        "<think>", "</think>", "（注意", "(注意", "注意：", "注：", "我需要", "首先", "用户说", "用户问",
        "用户刚才", "根据角色", "关键点", "思考：", "分析：", "回应要", "[旁白", "（旁白", "旁白：",
    ];
...
fn build_prompt(settings: &ChatSettings, memories: &str, history: &[ChatMessage], text: &str, now: &Situation) -> Vec<ModelMessage> {
    ...
    // 使唤的结果贴在用户这句话后面，而不是埋在系统提示里：9B 模型对紧挨着的旁白服从得多——
    // 放系统提示里时，「你已经拒绝了」也能被它说成「那我勉为其难答应吧」
    let content = match now.note.as_ref() {
        Some(n) => format!("{text}\n\n[旁白：{n}]"),
        None => text.into(),
    };
    result.push(ModelMessage { role: "user".into(), content });
    result
}
...
pub async fn send_chat_message(app: AppHandle, text: String, request_id: String) -> Result<ChatMessage, String> {
    ...
    // 别让「她明明去玩了」配上一条红色的错误
    if let (Some(fallback), Some(_)) = (now.fallback.as_ref(), error.as_ref()) {
        if status != "cancelled" {
...
/// 执行一句使唤，返回写给模型的「此刻」备注和兜底台词
fn handle_intent(app: &AppHandle, i: &Intent) -> (String, Option<String>) {
    match i {
        Intent::Do { target, minutes } => {
            let what = intent::describe_target(target);
            match crate::request_action_quiet(app, target.clone(), *minutes) {
                ...
                Some(v) => {
                    let why = match v.refusal {
                        Some(crate::core::obey::Refusal::Impossible) => "你现在做不来这件事",
                        Some(crate::core::obey::Refusal::Unknown) => "你不会做这件事",
                        Some(crate::core::obey::Refusal::Hungry) => "你太饿了",
                    ...
                    let note = format!(
                        "你已经拒绝了{what}，原因是{why}，现在没有去。用自己的话委婉地说明为什么这会儿不去；这次不能答应，也不要说「勉为其难同意」之类的话。"
                    );
                    (note, Some(v.say))
                }
        ...
        Intent::Bias { tag, weight } => {
            ...
            let note = if *weight > 0.0 {
                format!("你记下了：接下来几个小时会多{label}一些。简短回应就好。")
            } else {
                format!("你记下了：接下来几个小时会少{label}一些。简短回应就好。")
            };
            let fallback = if *weight > 0.0 { format!("好，我会多{label}一点的。") } else { format!("知道啦，我少{label}一点。") };
            (note, Some(fallback))
        }
    }
}

/// 一次性的短生成（台词、几句候选）：同一个模型、同样的预填充和收尾清理，不进对话记录
pub async fn complete(settings: &ChatSettings, system: &str, user: &str, num_predict: u32, temperature: f32) -> Result<String, String> {
    let name = &settings.persona.name;
    let prompt = wire_messages(
        &[
            ModelMessage { role: "system".into(), content: system.into() },
            ModelMessage { role: "user".into(), content: user.into() },
        ],
        name,
    );
    ...
    let mut output = String::new();
    generate_with(&client, &tuned, &prompt, num_predict, &mut output, |_| {}).await?;
    Ok(clean_reply(&output, name))
}
...
async fn generate(client: &Client, settings: &ChatSettings, prompt: &[ModelMessage], output: &mut String, on_delta: impl FnMut(&str)) -> Result<(), String> {
    generate_with(client, settings, prompt, 1024, output, on_delta).await
}

async fn generate_with(client: &Client, settings: &ChatSettings, prompt: &[ModelMessage], num_predict: u32, output: &mut String, mut on_delta: impl FnMut(&str)) -> Result<(), String> {
...
mod tests {
    ...
    fn export_only_confirmed_or_corrected_complete_model_answers_and_snapshot() {
        let db = store();
        let prompt = wire_messages(&build_prompt(&ChatSettings::default(), "[记忆上下文]", &[], "今天好吗？", &Situation::default()), "萝莉斯");
    ...
    fn transcripts_are_separate_from_memory_and_failures_do_not_enter_context() {
        ...
        let prompt = build_prompt(&ChatSettings::default(), "", &db.messages(100).unwrap(), "new", &Situation::default());
    ...
    fn history_is_bounded_and_pairs_stay_intact() {
        ...
        let prompt = build_prompt(&ChatSettings::default(), "", &history, "现在", &Situation::default());
    ...
    fn wire_messages_prefill_and_reply_cleanup() {
        let wire = wire_messages(&build_prompt(&ChatSettings::default(), "", &[], "在吗", &Situation::default()), "萝莉斯");
    ...
    fn live_local_model_replies_in_character() {
        ...
        for user in ["我今天把一个拖了很久的 bug 修好了，有点开心", "你是真人吗？能帮我打开浏览器吗", "今天想和你聊聊"] {
            let prompt = wire_messages(&build_prompt(&settings, "", &[], user, &Situation::default()), &settings.persona.name);
...
", raw.chars().count(), deltas);
    ...
    #[test]
    fn situation_goes_into_the_system_prompt() {
        let now = Situation {
            state: describe_state(&PetState::default()),
            note: Some("用户刚才让你去玩，你答应了".into()),
            fallback: None,
        };
        let prompt = build_prompt(&ChatSettings::default(), "", &[], "去玩吧", &now);
        ...
        let plain = build_prompt(&ChatSettings::default(), "", &[], "在吗", &Situation::default());
        assert!(!plain[0].content.contains("# 此刻"));
        assert_eq!(plain.last().unwrap().content, "在吗");
    }

    #[test]
    fn describe_state_reads_like_her() {
        let mut s = PetState::default();
        s.hunger = 10.0;
        s.strength = 30.0;
        let d = describe_state(&s);
        assert!(d.contains("饿得不行"), "{d}");
        assert!(d.contains("有点累"), "{d}");
        assert!(d.starts_with("你正闲着"));
    }
